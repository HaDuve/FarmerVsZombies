    ...
    internal sealed class UnitSelectionBox
    {
        ...
        public static bool Select(IEnumerable<BGameObject> list, bool box, Vector2 mousePosition)
        {
            if (box)
            {
                ...
                }else if (selected is Tower)
                {
                    if (selected.Position.Y > (mousePosition.Y))
                    {
                        selected.Deselect();
                        return false;
                    }
                }
                else if (selected is Tree)
                {
                    if (selected.Position.Y > (mousePosition.Y) + 2 && ((selected.Position.X > (mousePosition.X)) || (selected.Position.X + 1 < (mousePosition.X))))
                    {
                        selected.Deselect();
                        return false;
                    } else if (selected.Position.Y < (mousePosition.Y) && ((selected.Position.X > (mousePosition.X)) || (selected.Position.X + 1 < (mousePosition.X))))
                    {
                        selected.Deselect();
                        return false;
                    }
