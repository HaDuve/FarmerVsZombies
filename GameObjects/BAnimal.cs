...
namespace FarmervsZombies.GameObjects
{
    internal abstract class BAnimal : BMovableGameObject, IAttackable, ICollidable
    {
        ...
        private float mTimeSinceFed;
        private const float TimeUntilHunger = 18f;
        private const float StarveCooldown = 12f;
        private float mTimeSinceStarve;
        private int mFoodAvailable;
        ...
        protected virtual void Eat()
        {
            ...
            if (mFoodAvailable == 0) Feed(mFeedAction);
            if (mFoodAvailable <= 0) return;
            Health += 10;
            mTimeSinceFed = 0f;
            mResourcesHeld++;
        ...
        private void Feed(string feedAction)
        {
            if (!EconomyManager.Instance.SeedDecrease(feedAction, false)) return;
            mFoodAvailable += 5;
        }

        private void Starve(string starveText)
        {
            if (mTimeSinceFed < TimeUntilHunger || mTimeSinceStarve < StarveCooldown) return;
            mTimeSinceStarve = 0;
            Health -= 20;
            if (Health < 0) base.Death();
            NotificationManager.AddNotification(starveText, 5);
        }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            Starve(mStarveNotificationText);

            mTimeSinceEat += gameTime.ElapsedGameTime.Milliseconds / 1000f;
            mTimeSinceStarve += gameTime.ElapsedGameTime.Milliseconds / 1000f;
            mTimeSinceFed += gameTime.ElapsedGameTime.Milliseconds / 1000f;
        }
    }
}
