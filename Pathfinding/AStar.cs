...
namespace FarmervsZombies.Pathfinding
{
    internal sealed class AStar : IPathfinder
    {
        ...
        {
            for (int i = (int)Math.Floor(target.X - 0.35f); i < Math.Ceiling(target.X + 0.35f); i++)
            {
                for (int j = (int)Math.Floor(target.Y - 0.35f); j < Math.Ceiling(target.Y + 0.35f); j++)
                {
                    if (GameGraph.GameGraph.Graph.CheckCollision(i, j))
                        return false;
                }
            }
