...
namespace FarmervsZombies
{
    ...
    internal sealed class Game1 : Game
    {
        ...
        private readonly OptionsMenu mOptions = new OptionsMenu();
        ...
        private readonly PauseMenu mPause = new PauseMenu();
        ...
        public static bool TechDemoActive { get; set; }
        ...
        public Game1()
        {
            ...
            // Camera
            mCamera = new Camera(GraphicsDevice.Viewport);
        }
        ...
        protected override void Initialize()
        {
            ...
            mOptions.Initialize(mGraphics.GraphicsDevice);
        ...
        protected override void Update(GameTime gameTime)
        {
            switch (GameStateManager.State)
            {
                case Menu.GameState.PlayGameMenu:
                    ...
                    if (!GraveyardBuilt)
                    {
                        ...
                        GraveyardBuilt = true;
                        TechDemoActive = false;

                        SpawnGraveyardTreasures();
                    }
                    
                    ObjectManager.Instance.Update(gameTime);
                    if (!TechDemoActive) Ai.Update(gameTime);
                    sTileMap.Update(gameTime);

                    // Update Camera
                    mCamera.UpdateCamera(GraphicsDevice.Viewport, InputManager.GetCurrentInputState());
                    // Update Economy
                    ...
                    if (sStatistics.OldGameTime + mTime >= 300 && !sAchievements.DasWarErstDerAnfang && !TechDemoActive)
                    {
                    ...
                    } else if (sStatistics.OldGameTime + mTime >= 3000 && !sAchievements.DieLangeNacht && !TechDemoActive)
                    {
            ...
            if (sResolutionChanged)
            {
                ...
                mResolutionMenu.UpdateButtons();
                mHudMenu.UpdateButtons();
                mAchievements.UpdateButtons();
                mMain.UpdateButtons();
                mLevelMenu.UpdateButtons();
                mStartGame.UpdateButtons();
                mStatistics.UpdateButtons();
                mPause.UpdateButtons();
                sLoadGame.UpdateButtons();
        ...
        private static void SpawnNecromancer(object sender, InputState inputState)
        {
            ...
            if (sTileMap.GetTileType((int)farmer.Position.X + 3, (int)farmer.Position.Y + 3) == Tile.Grass)
            {
                var necromancer = new Necromancer(farmer.Position.X + 3, farmer.Position.Y + 3);
        ...
        private void PauseGame(object sender, InputState inputState)
        {
            switch (GameStateManager.State)
            {
                case Menu.GameState.PlayGameMenu:
                    ...
                    break;
                case Menu.GameState.PauseMenu:
                    if (mPause.ShowTextBox)
                    {
                        mPause.ShowTextBox = !mPause.ShowTextBox;
                    }
                    else
                    {
                        GameStateManager.State = Menu.GameState.PlayGameMenu;
        ...
        {
            if (!sEndScreen) SetupEndScreen(win);
        ...
        private void SetupEndScreen(bool win)
        {
            sEndScreen = true;
            ...
            mCamera.Lock();
            SoundManager.PlaySong("game_over", 0.7f);
