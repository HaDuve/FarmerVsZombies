    ...
    internal sealed class SteeringManager
    {
        ...
        private void Collide()
        {
            ...
            foreach (var waterTile in mSteeringGameObject.GetCollidingWaterTiles())
            {
                ...
                var collisionForce = GetWaterCollisionForce(waterTile);
                if (collisionForce != Vector2.Zero) distance = collisionForce * distance.Length();
        ...
        private Vector2 GetWaterCollisionForce(Vector2 waterTile)
        {
            var collisionForce = Vector2.Zero;
            for (var i = Math.Max(0, (int)waterTile.X - 1); i <= Math.Min(waterTile.X + 1, Game1.MapWidth); i++)
            {
                for (var j = Math.Max(0, (int)waterTile.Y - 1); j <= Math.Min(waterTile.Y + 1, Game1.MapHeight); j++)
                {
                    if (Game1.sTileMap.GetTileType(i, j) != Tile.Water)
                    {
                        collisionForce += Vector2.Normalize(new Vector2(i, j) - waterTile);
                    }
                }
            }
            if (collisionForce != Vector2.Zero) collisionForce.Normalize();
