...
namespace FarmervsZombies.Menus
{
    internal sealed class TileMenu : ActionMenu
    {
        ...
        private List<ActionMenuButton> GetButtons()
        {
            ...
            switch (mTileMap.GetTileType((int)mWorldPosition.X, (int)mWorldPosition.Y))
            {
                ...
                case Tile.Dirt:
                    ...
                    break;
                case Tile.Water:
                    if (Game1.CheatMode) result.Add(new ActionMenuButton(mWorldPosition, TextureManager.GetTexture("icons", 32, 32, 32), ActionPlantGrass, "Gras pflanzen"));
                    break;
                case Tile.Wasteland:
        ...
        private void ActionBuildFarmhouse()
        {
            ...
            if (mWorldPosition.X < 3 || mWorldPosition.X > Game1.sTileMap.Width - 5 || mWorldPosition.Y < 3 ||
                mWorldPosition.Y > Game1.sTileMap.Height - 5)
            {
            ...
            for (var i = (int)mWorldPosition.X - 1; i < (int)mWorldPosition.X + (int)farmhouse.CollisionBoxSize.X + 1; i++)
            {
                for (var j = (int)mWorldPosition.Y - 1; j < (int)mWorldPosition.Y + (int)farmhouse.CollisionBoxSize.Y + 1; j++)
                {
            ...
            if (ObjectManager.Instance.GetQuadTree().GetGameObjects(
                farmhouse.CollisionBoxCenter - farmhouse.CollisionBoxSize / 2 - Vector2.One,
                farmhouse.CollisionBoxCenter + farmhouse.CollisionBoxSize / 2 + Vector2.One,
                false,
